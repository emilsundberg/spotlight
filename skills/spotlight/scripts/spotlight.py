...
def git_path(root, name):
    value = os.fsdecode(git(root, 'rev-parse', '--git-path', name)).removesuffix('\n')
    path = Path(value)
...
def base_checkout(location):
    ...
    if not base.is_dir() or common_directory(base) != common_directory(location):
        raise SpotlightError('The registered main checkout is missing or belongs to another repository.')
    return base


def checkout(location):
    return Path(os.fsdecode(git(location, 'rev-parse', '--show-toplevel')).removesuffix('\n')).resolve()


def common_directory(location):
    return Path(os.fsdecode(git(location, 'rev-parse', '--path-format=absolute',
...
class Spotlight:
    ...
    def source(self, value=None, location=None):
        records = worktrees(self.base)
        if value is None:
            current = checkout(location or Path.cwd())
            if current != self.base:
                value = str(current)
            elif self.load():
                value = self.load()['source']
            else:
                candidates = [r['worktree'] for r in records[1:]
                              if 'prunable' not in r and Path(r['worktree']).is_dir()]
                if len(candidates) != 1:
                    raise SpotlightError('You are in the main checkout. Choose a worktree with '
                                         'spotlight on <branch-or-path>; see spotlight list.')
                value = candidates[0]
        path = Path(value).expanduser().resolve()
        if path.is_dir():
            path = checkout(path)
        matches = [Path(r['worktree']).resolve() for r in records
                   if 'prunable' not in r and (Path(r['worktree']).resolve() == path
                                              or r.get('branch') == 'refs/heads/' + value)]
        if len(matches) != 1 or not matches[0].is_dir():
            raise SpotlightError('Choose an existing worktree path or its exact branch name; see spotlight list.')
        source = matches[0]
        if checkout(source) != source or common_directory(source) != common_directory(self.base):
            raise SpotlightError('The source no longer belongs to this repository. Repair its worktree registration.')
    ...
    def on(self, value, once, interval, location=None):
        source = self.source(value, location)
        state = self.load()
    ...
    def status(self, as_json, location=None):
        state = self.load()
        current = checkout(location or self.base)
        result = {'active': bool(state), 'base': str(self.base), 'checkout': str(current),
                  'role': 'main' if current == self.base else 'worktree'}
        if state:
            alive = False
        ...
        if as_json:
            ...
            print(f"You are in the {result['role']} checkout: {current}\nSpotlight is off. Base: {self.base}")
        else:
...
def main(argv=None):
    ...
    parser.add_argument('--version', action='version', version='spotlight 0.2.0')
    ...
    attach.add_argument('worktree', nargs='?', help='Defaults to the current worktree; path or branch overrides it')
    ...
    try:
        location = args.base.expanduser().resolve() if args.base else Path.cwd()
        if args.command == 'on' and args.worktree and args.base is None:
            explicit_path = Path(args.worktree).expanduser().resolve()
            if explicit_path.is_dir():
                location = explicit_path
        base = base_checkout(location)
        ...
        with spotlight.lock():
            if args.command == 'list':
                ...
                spotlight.on(args.worktree, args.once, args.interval, location)
            elif args.command in ('status', None):
                spotlight.status(getattr(args, 'json', False), location)
